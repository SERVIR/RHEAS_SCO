...
class Model(DSSAT):
    ...
    def _writeFertilizer(self, fout, fertilizers):
        ...
        for f, fert in enumerate(fertilizers[0]):
    ...
    def writeControlFile(self, modelpath, vsm, depths, startdate, gid, lat, lon, planting, fertilizers, irrigation):
        ...
        for ens in range(self.nens):
            iplanting = planting
            sm = vsm[ens]
            planting = planting + timedelta(19)
            fertilizers = [(planting, "005", "001", 1.0, 50.0), (planting+timedelta(30), "005", "001", 1.0, 75.0)] if fertilizers is None else fertilizers
