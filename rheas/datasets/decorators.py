...
def netcdftam(fetch):
    ...
    def wrapper(*args, **kwargs):
        ...
        url = url+'/rfe'+yr+'_'+m+'_'+d+'.v3.1.nc'
        ds = netcdf4.Dataset(url)
        for var in ds.variables:
            if var.lower().startswith("lon") or var.lower() == "x":
                lonvar = var
        ...
        t = ds.variables[timevar]
        tt = netcdf4.num2date(t[:], units=t.units)
        ti = [tj for tj in range(len(tt)) if resetDatetime(tt[tj]) >= dt and resetDatetime(tt[tj]) <= dt]
        if len(ti) > 0:
            lati = np.argsort(lat)[::-1][i1:i2]
            loni = np.argsort(lon)[j1:j2]
            if len(ds.variables['rfe'].shape) > 3:
                data = ds.variables['rfe'][ti, 0, lati, loni]
            else:
                data = ds.variables['rfe'][ti, lati, loni]
            dt = tt[ti]
        else:
            data = None
            dt = None
