...
edited to import NMME data from a file
"""
...
def download(dbname, dts, bbox=None):
    ...
    for varname in varnames:
        for e in range(nens):
            ...
            climateserv.api.request_data("Seasonal_Forecast", "Download", dt1, dt2, coords, "ens{0:02d}".format(e+1), varname, zfilename)
            ...
            for filename in filenames:
                ...
                if varname == "Temperature":
                    ...
                    log.debug(out)
                    filename = "C" + filename
                    proc = subprocess.Popen(["gdal_edit.py", "-a_nodata", "nan", "{0}/{1}".format(outpath, filename)])
                    out, err = proc.communicate()
                    log.debug(out)
                ingest(dbname, varname, "{0}/{1}".format(outpath, filename), dt, e+1)
                os.remove("{0}/{1}".format(outpath, filename))
